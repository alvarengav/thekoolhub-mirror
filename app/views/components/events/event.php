...
$isOldDate = $eventDate < $currentDate;
?>
<div class="box-darkblue box  <?= isset($style) ? 'box-' . $style : '' ?>"
    style="<?= $isOldDate === true ? 'opacity: .5' : ''  ?>">
    <div class="picture">
        <img src="<?= upload($event['file']) ?>">
        ...
        <div class="center">
            ...
            <div class="info">
                ...
                <div class="time">
                    ...
                    <?= date_to_time($event['date']) ?>
                    -
                    <?= date_to_time($event['end_date']) ?>
                </div>
            </div>


            <div class="text">
                <?= nl2br($event['subtitle']) ?>
            </div>

            <div class="info">


                <a href="<?= $event['link'] ? $event['link'] : '' ?>" class="register"
                    <?= isset($event['blank']) && $event['blank'] == '1' ? ' target="_blank"' : '' ?>>
                    <b><?= $this->Data->lang('Regístrate') ?></b>
                ...
                <a class="download" href="<?= upload($event['file_down']) ?>"
                    download="<?= $event['file_name_down'] ?>">
                    <? $this->load->view('components/svgs/download.svg') ?>
                    <b><?= $this->Data->lang('Descargar info') ?></b>
                    <? endif ?>
