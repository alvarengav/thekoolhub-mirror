...
// 		 * It can either set to GMT or time(). The pref
		...
		{
			$this->sess_cookie = $this->object->config->item('sess_cookie_name').$this->object->config->item('cookie_prefix');
		}
    ...
    $session_id = $this->object->encryption->decode($session_ide);
	...
	function sess_read( $session_id = 0 )
  {
    ...
    $session_id = $this->object->encryption->encode($this->userdata['session_id']);
		setcookie(
					$this->sess_cookie, 
					$session_id,
