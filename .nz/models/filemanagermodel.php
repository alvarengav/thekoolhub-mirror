<?php
class FileManagerModel extends AppModel
{
  ...
  public function uploadFile( $file = null )
  {
    ...
    $max_size = round($this->input->post('max_size'));
    if($max_size)
    {
      $data['url_max_size'] = thumb($name, $max_size, $max_size, false, $this->fglobal);
    }
  ...
  public function generateThumb( $image = '', $name = '', $width = 0, $height = 0 )
  {
    ...
    $this
      ...
      ->load($image)
      ->resize($width,$height)
      ->save($this->getFolder()."thumb/".$name)
      ->clear();
  }
