...
class MembersModel extends AppModel {
  ...
  private function ListWhere($filter = false)
  {
    ...
    if($this->input->post('filter-active'))
      $sql .= " AND t.active = '1'";
    if($text)
      $sql .= " AND ( t.title like '%{$text}%'  OR  t.sector like '%{$text}%'  OR  t.lang like '%{$text}%'  OR t.id_post = '{$text}') ";   
  ...
  public function DataElement( $id = 0, $null = false)
  {
    ...
    $ret['sector'] = $this->input->post() ? $this->input->post('sector') : '';
