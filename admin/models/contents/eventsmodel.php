...
class EventsModel extends AppModel {
  ...
  public function SavePost()
  {
    ...
    $data = array(
      ...
      'id_file_down' => $this->input->post('id_file_down'),
  ...
  public function DataElement( $id = 0, $null = false)
  {
    ...
    if($id)
    {
      ...
      FROM {$this->table} as t      
      LEFT JOIN nz_file lj0 on t.id_file = lj0.id_file      
      LEFT JOIN nz_file lj1 on t.id_file_down = lj1.id_file      
      WHERE t.id_post = '{$id}' 
      LIMIT 0, 1";
    ...
    $ret['lang'] = $this->input->post() ? $this->input->post('lang') : '';
    $ret['active'] = $this->input->post('active') ? 1 : 0;
    $ret['id_file'] = $this->input->post() ? $this->input->post('id_file') : '';
    $ret['id_file_down'] = $this->input->post() ? $this->input->post('id_file_down') : '';
    ...
    $ret['blank'] = $this->input->post() ? $this->input->post('blank') : '';
    $ret['end_date'] = $this->input->post() ? $this->input->post('end_date') : '';
    return $ret;
  }
  // ALTER TABLE `events` ADD `link` VARCHAR(255) NULL AFTER `id_file_down`
  // ALTER TABLE `events` ADD `blank` BOOLEAN NOT NULL ;
