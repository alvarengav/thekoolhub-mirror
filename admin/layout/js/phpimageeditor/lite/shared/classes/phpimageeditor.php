	...
	class PHPImageEditor 
	{
		...
		function Action($actionType)
		{
			...
			if ($actionType == $this->actionSaveAndClose)
			{
				...
				$newFile = $this->srcEdit;
				$info = pathinfo($this->srcEdit);
 				$newFile = $info['dirname'] .'/'. $info['filename'] . '_' . time() . ( $info['extension'] ? '.'. $info['extension'] : '');
				$this->SaveImage($this->resourceWorkWith, $newFile, $this->mimeType);
				...
				if($idf)
				{
					PIE_Echo('<script language="javascript" type="text/javascript">window.opener.refreshFileInputG("' . $form . '", "' . $newFile . '", "' . $idf . '");'.$reloadParentBrowser.'window.open(\'\',\'_parent\',\'\');window.close();</script>');					
				}
				else
				{
					PIE_Echo('<script language="javascript" type="text/javascript">window.opener.refreshFileInput("' . $form . '", "' . $newFile . '");'.$reloadParentBrowser.'window.open(\'\',\'_parent\',\'\');window.close();</script>');
      ...
      { 
		...
		function GetFormAction()
		{
			...
			$idf = isset($_GET['idf']) ? 'idf='.$_GET['idf'].'&' : '';
			$sizeX = isset($_GET['sizeX']) ? 'sizeX='.$_GET['sizeX'].'&' : '';
			$sizeY = isset($_GET['sizeY']) ? 'sizeY='.$_GET['sizeY'].'&' : '';

			return "index.php?".$form.$idf.$sizeX.$sizeY.$joomlaIsAdmin.$this->httpImageKey."=".urlencode($this->srcEdit);
		}
