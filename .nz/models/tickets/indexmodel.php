...
class IndexModel extends AppModel {
  ...
  private function ListWhere($filter = false)
  {
    ...
    $ids = array(0);
    if($this->input->post('filter-client'))
    {    	
	    $sqlx = "SELECT p.id_project as id
    ...
    {
	    $sqlx = "SELECT p.id_client
	    FROM {$this->dbglobal}project p
	    where p.id_project = '{$project}'";
	    $projectRow = $this->db->query($sqlx)->row();
	    if($projectRow)
	    {
	    	$sqlx = "SELECT p.id_project as id
	   		FROM {$this->dbglobal}project p
	   		left join {$this->dbglobal}project_company s on s.id_project = p.id_project
	   		where p.id_client = '{$projectRow->id_client}'";
	    	$result = $this->db->query($sqlx)->result();
	    	foreach($result as $r)
	      	$ids[] = $r->id;
	    }    	
