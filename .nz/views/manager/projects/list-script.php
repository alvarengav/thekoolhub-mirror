...
var DataTableFn = function(){
  ...
  configDT.fnServerParams = function ( aoData ) {
    ...
    if($('#finishFormChk<?= $wgetId ?>').prop('checked'))
      aoData.push( { "name": "filter-finish", "value": 1 } );
  ...
  configDT.aoColumns = [
    ...
    { "sWidth": "80px", "sTitle": "<?= $this->lang->line("Cliente") ?>", "mData": "client", "sType": "html", "mRender" : function( data, type, full ){ 
      if(!full['id_client']) return '-';
      return '<a class="app-loader" href="<?= base_url() ?>manager/clients/element/' + full['id_client'] + '/back?uri=<?= current_url() ?>">' + full['client'] + '</a>';
    }},
    { "sTitle": "<?= $this->lang->line("Proyecto") ?>", "mData": "project", "sType": "html", "mRender" : function( data, type, full ){ 
      if(full['id'] != <?= (string)$this->MApp->project ?>) return data;      
      return data + '<i data-title="<?= $this->lang->line("Proyecto actual") ?>" class="ico-project-active fa fa-exclamation-triangle tooltip-nz-app ttactive"></i>';
    }},
    { "sClass": "text-align-center", "sTitle": "<?= $this->lang->line("Inicio") ?>", "mData": "date", "sType": "html", "mRender" : function( data, type, full ){ 
      if(!data || data == '0000-00-00') return '-';
      return Date.fromMysql(data).format("dd/MM/yyyy");
    }},
    { "sClass": "text-align-center", "sTitle": "<?= $this->lang->line("URL") ?>", "mData": "url", "sType": "html", "mRender" : function( data, type, full ){ 
      if(!data) return '-';
    ...
    { "sWidth": "60px", "sClass": "text-align-center widget-filemanager", "sTitle": "<?= $this->lang->line("Imagen") ?>", "mData": "fm1file", "sType":"html", "mRender" : function( data, type, full ){ 
      ...
      return (data ? '<a class="no-propagation" href="<?= upload() ?>'+ full["fm1file"] +'" target="_blank">' : '') + '<div data-type="'+type +'" class="file-info type-'+ type +'"><div class="file-ico">' + ((data  && type == 1) ? '<img src="<?= thumb_url('', true) ?>'+ full["id_file"] +'<?= thumb_version() ?>" />' : '' ) +'</div></div>' + (data ? '</a>' : '');
    }},
    { "sClass": "text-align-center", "sWidth": "40px", "sTitle": "<?= $this->lang->line("Empresas") ?>", "mData": "companies", "sType": "html", "mRender" : function( data, type, full ){ 
      if(!data || !parseInt(data)) return '-';
      return parseInt(data);
    }},
    { "sClass": "text-align-center", "sWidth": "40px", "sTitle": "<?= $this->lang->line("Terminado") ?>", "mData": "finish", "sType": "html", "mRender" : function( data, type, full ){ 
