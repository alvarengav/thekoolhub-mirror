...
class ComponentUpdated {
  ...
  public function update( $opts = array(), $init = false )
  {

    foreach($this as $key => $value)
    {
      if( isset($opts[$key]) && !is_array($value))
        $this->$key = quest_sanitize($opts[$key]);
  ...
  function decode( $str = '')
  {
    ...
    foreach($datos as $v)
    {
      ...
      if( count($arr) == 2 )
      {
        ...
        $ret[$arr[0]] = isset($arr[1]) ? quest_sanitize(urldecode($arr[1])) : "";
      }elseif( count($arr) > 2)
      {
        ...
        {
          $arr2 = explode(':',$t);
          $ret[$arr[0]][$arr2[0]] = isset($arr2[1]) ? quest_sanitize(urldecode($arr2[1])) : "";
        }
      }
    }
    return $ret;
